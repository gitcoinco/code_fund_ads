...
const StackedMenu = require('stacked-menu')
    ...
    _createClass(Theme, [
      ...
      },
      {
        ...
        value: function togglePasswordVisibility () {
          $(document).on('click', '[data-toggle="password"]', function (e) {
            ...
            $(this)
              .children('.fa, .far')
              .toggleClass('fa-eye fa-eye-slash', hasFa) // toggle trigger text

            $(this)
              .children()
              .last()
              .text(triggerText) // toggle input type
      ...
      },
      {
        ...
        value: function modalScrollable () {
          ...
          $(
            '.modal-dialog-scrollable .modal-body, .modal-drawer .modal-body'
          ).on('scroll', function () {
            var $elem = $(this)
            var elem = $elem[0]
        ...
        value: function autofocusInputBehaviour () {
          $(document).on(
            ...
            function (e) {
              ...
              $modal
                .find(
                  'input[autofocus]:first, input[data-autofocus="true"]:first'
                )
                .focus()
            }
          )
      ...
      },
      {
        ...
        value: function pageExpander () {
          $(document).on('click', '[data-toggle="page-expander"]', function (
            e
          ) {
            e.preventDefault()
          ...
          if (window.Chart) {
            ...
            var settings = {
              global: {
                ...
                defaultColor: isDarkSkin
                  ? this.hexToRgba(colors.white, 0.08)
                  : this.hexToRgba(colors.black, 0.1),
                defaultFontColor: isDarkSkin ? gray[400] : gray[600],
                fontFamily:
                  '-apple-system, BlinkMacSystemFont, "Fira Sans", "Helvetica Neue", "Apple Color Emoji", sans-serif',
                tooltips: {
                  backgroundColor: isDarkSkin
                    ? this.hexToRgba(colors.white, 0.98)
                    : this.hexToRgba(colors.black, 0.98),
                  xPadding: 8,
                  yPadding: 8,
                  titleMarginBottom: 8,
                  footerMarginTop: 8,
        ...
        value: function bootstrapSelect () {
          if ($.fn.selectpicker) {
            ...
            $('[data-toggle="selectpicker"]').each(function () {
              ...
              $(selector)
                .selectpicker() // add dropdown menu arrow
          ...
          if (window.Tribute) {
            $('[data-toggle="tribute"]').each(function () {
              ...
              options.menuContainer =
                document.querySelector(options.menuContainer) || false // define custom template
              ...
              if (options.itemTemplate == true) {
                options.menuItemTemplate = function (item) {
                  return '<span class="user-avatar user-avatar-sm mr-2"><img src="'
                    .concat(item.original.avatar, '"></span> ')
                    .concat(item.string)
                }
              ...
              if (options.selectTemplate == true) {
                options.selectTemplate = function (item) {
                  ...
                  return '<a href="#" class="mention">@'.concat(
                    item.original.value,
                    '</a>'
                  )
                }
          ...
          if (window.vanillaTextMask) {
            $('[data-mask]').each(function () {
              ...
              if (options.autoCorrectDate) {
                var autoCorrectedDatePipe = textMaskAddons.createAutoCorrectedDatePipe
          ...
          if (window.zxcvbn) {
            $('.form-strength-meter').each(function () {
              ...
              $(input).on('keyup', function () {
                ...
                var indicatorWidth = ''.concat(
                  ((result.score + 1) / strength.length) * 100,
                  '%'
                ) // Update the password strength meter

                if (val !== '') {
                  $(indicator)
                    .removeClass('d-none '.concat(strength.join(' ')))
                    .addClass(''.concat(strength[result.score]))
                    .css('width', indicatorWidth)
                  $(feedback).html(
                    '<strong>'
                      .concat(result.feedback.warning, '</strong> ')
                      .concat(result.feedback.suggestions)
                  )
                } else {
